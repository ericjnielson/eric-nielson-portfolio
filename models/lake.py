...
class FrozenLake:
    ...
    def generate_path_visualization(self):
        """Visualize the environment with optimal path using PIL"""
        # Get raw frame
        ...
        if self.optimal_path:
            ...
            for i in range(len(self.optimal_path) - 1):
                ...
                radius = 5
                draw.ellipse(
                    [start_point[0]-radius, start_point[1]-radius, 
                     start_point[0]+radius, start_point[1]+radius], 
                    fill=(0, 255, 0))
            ...
            if self.optimal_path:
                ...
                radius = 5
                draw.ellipse(
                    [final_point[0]-radius, final_point[1]-radius, 
                     final_point[0]+radius, final_point[1]+radius], 
                    fill=(0, 0, 255))
    ...
    def get_metrics(self):
        ...
        return {
            'average_reward': float(np.mean(recent_rewards)),  # Convert to float for JSON serialization
            'episode_count': self.current_episode,
            'success_rate': float(self.success_rate),  # Convert to float for JSON serialization
            'epsilon': float(self.epsilon),  # Convert to float for JSON serialization
            'training_complete': self.training_complete,
            'best_path_length': int(self.best_path_length) if self.best_path_length != float('inf') else 0
        }
